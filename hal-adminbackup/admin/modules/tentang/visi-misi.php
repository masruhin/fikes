...
<?php if (isset($_GET['saved'])): ?>
  <div class="alert success">Visi dan misi berhasil disimpan.</div>
<?php endif; ?>
<form method="post">
  ...
  <div class="panel">
    ...
    <div id="misiWrap">
      <?php foreach ($misi as $i => $m): ?><div class="mission-row">
          <div class="mission-number"><?= sprintf('%02d', $i + 1) ?></div>
          <div>
            <input name="judul[]" value="<?= e($m['judul']) ?>" placeholder="Judul misi">
            <textarea name="isi[]" rows="3" placeholder="Isi misi"><?= e($m['isi']) ?></textarea>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
  ...
  function addMission() {
    ...
    document.getElementById('misiWrap').insertAdjacentHTML('beforeend',
      `<div class="mission-row">
        <div class="mission-number">${String(n).padStart(2,'0')}</div>
      <div>
        <input name="judul[]" placeholder="Judul misi">
        <textarea name="isi[]" rows="3" placeholder="Isi misi"></textarea>
      </div>
    </div>`
    )
  }
