...
if (isset($_GET['pesan'])) {
    ...
    if ($_GET['pesan'] === 'ukuran') { $pesan = 'Ukuran gambar maksimal 3 MB.'; $tipe = 'danger'; }
}
...
<div class="panel">
    ...
    <table>
        <thead><tr><th>No</th><th>Gambar</th><th>Periode</th><th>SK Rektor</th><th>Dekan</th><th>Wakil Dekan Akademik</th><th>Wakil Dekan Adum & Keu</th><th>Wakil Dekan Kemahasiswaan</th><th>Aksi</th></tr></thead>
        <tbody>
        <?php if (!$data): ?>
            <tr><td colspan="9" style="text-align:center;color:#8993a5;padding:35px;">Belum ada data struktur organisasi.</td></tr>
        <?php else: ?>
            ...
            <tr>
                ...
                <td>
                    <?php if (!empty($row['gambar'])): ?>
                        <a href="<?= e($upload_url . $row['gambar']) ?>" target="_blank"><img src="<?= e($upload_url . $row['gambar']) ?>" alt="Struktur" style="width:90px;height:60px;object-fit:cover;border-radius:7px;border:1px solid #e8ecf3;"></a>
                ...
                <td><a href="?edit=<?= (int)$row['id'] ?>" class="btn small">Edit</a> <a href="?hapus=<?= (int)$row['id'] ?>" class="btn small danger-text" onclick="return confirm('Hapus data struktur periode <?= e($row['periode']) ?>?')">Hapus</a></td>
            </tr>
            <?php endforeach; ?>
        <?php endif; ?>
        </tbody>
...
<div class="modal <?= $edit ? 'show' : '' ?>" id="modalStruktur">
    <div class="modal-box large">
        <div class="modal-head"><div><span class="eyebrow">FORM DATA</span><h2><?= $edit ? 'Edit Struktur Organisasi' : 'Tambah Struktur Organisasi' ?></h2></div><button type="button" onclick="tutupModalStruktur()">×</button></div>
        <form method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= (int)($edit['id'] ?? 0) ?>">
            <div class="form-grid">
                <div><label>Periode</label><input type="text" name="periode" value="<?= e($edit['periode'] ?? '2024 - 2026') ?>" required></div>
                <div><label>SK Rektor</label><input type="text" name="sk_rektor" value="<?= e($edit['sk_rektor'] ?? 'Nomor 030/Univ.BHAMADA/KEP/V/2024') ?>" required></div>
                <div><label>Dekan</label><input type="text" name="dekan" value="<?= e($edit['dekan'] ?? 'Rosmalia, S.T.,M.Kes.') ?>" required></div>
                <div><label>Wakil Dekan Bidang Akademik</label><input type="text" name="wakil_dekan_akademik" value="<?= e($edit['wakil_dekan_akademik'] ?? 'Siswati, S.Si.T.,Bdn.,M.Kes.') ?>" required></div>
                <div><label>Wakil Dekan Bidang Adum & Keu</label><input type="text" name="wakil_dekan_adum_keu" value="<?= e($edit['wakil_dekan_adum_keu'] ?? 'Sri Hidayati, Ns.,M.Kep.,Sp.Kep.MB.') ?>" required></div>
                <div><label>Wakil Dekan Bidang Kemahasiswaan</label><input type="text" name="wakil_dekan_kemahasiswaan" value="<?= e($edit['wakil_dekan_kemahasiswaan'] ?? 'Deni Irawan, Ns.,M.Kep.') ?>" required></div>
                <div class="full"><label>Gambar Struktur Organisasi</label><input type="file" name="gambar" accept=".jpg,.jpeg,.png,.webp"><small style="display:block;color:#8993a5;margin-top:-8px;margin-bottom:12px;">Format JPG, JPEG, PNG, WEBP. Maksimal 3 MB.</small>
                    <?php if (!empty($edit['gambar'])): ?><img src="<?= e($upload_url . $edit['gambar']) ?>" alt="Gambar struktur" style="max-width:100%;max-height:280px;border:1px solid #e8ecf3;border-radius:10px;"><?php endif; ?>
                </div>
            </div>
            <div class="modal-actions"><button type="button" class="btn" onclick="tutupModalStruktur()">Batal</button><button type="submit" class="btn primary">💾 Simpan Data</button></div>
        </form>
    </div>
</div>
